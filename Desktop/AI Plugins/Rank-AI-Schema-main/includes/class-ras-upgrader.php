 ...
 * stored DB version differs from RAS_VERSION.
 *
 * Adding a new migration:
 *   1. Bump RAS_VERSION in rank-ai-schema.php.
...
class RAS_Upgrader {
    ...
    const DB_VERSION_KEY = 'ras_db_version';

    /** Option key for the full upgrade history log. */
    const HISTORY_KEY    = 'ras_upgrade_history';
    ...
    public static function maybe_upgrade() {
        ...
        if ( version_compare( $installed, RAS_VERSION, '>=' ) ) {
            return;
        }
    ...
    private static function run_migrations( $from_version ) {
        ...
        foreach ( self::$migrations as $version => $method ) {
            ...
            try {
                ...
                error_log( '[Rank AI Schema] Migration error for ' . $version . ': ' . $e->getMessage() );
            }
        }

        // Update stored DB version.
        update_option( self::DB_VERSION_KEY, RAS_VERSION, false );
    ...
    private static function migrate_2_0_0() {
        ...
        if ( ! get_option( RAS_SEO_Analyzer::OPTION_SUMMARY ) ) {
            update_option( RAS_SEO_Analyzer::OPTION_SUMMARY, [
    ...
    private static function migrate_2_1_0() {
        $settings = get_option( 'ras_global_settings', [] );
        $changed  = false;
        ...
        if ( $changed ) {
            update_option( 'ras_global_settings', $settings );
        }
    ...
    private static function migrate_2_2_0() {
        ...
        $deleted = $wpdb->query(
            "DELETE FROM {$wpdb->postmeta}
             WHERE meta_key IN ('_ras_seo_score', '_ras_seo_results', '_ras_seo_ts')"
        );
    ...
    public static function uninstall() {
        ...
        // Options.
        $option_keys = [
            'ras_global_settings', 'ras_version', self::DB_VERSION_KEY,
            self::HISTORY_KEY, RAS_SEO_Analyzer::OPTION_SUMMARY,
        ];
        foreach ( $option_keys as $key ) {
            delete_option( $key );
        }

        // Post meta — all _ras_ prefixed keys.
        $wpdb->query(
