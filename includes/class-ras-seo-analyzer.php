...
class RAS_SEO_Analyzer {
    ...
    public static function analyze( $post_id ) {
        ...
        $og_t       = (string) get_post_meta( $post_id, '_ras_og_title',  true );
        $og_d       = (string) get_post_meta( $post_id, '_ras_og_desc',   true );
        $og_i       = (string) get_post_meta( $post_id, '_ras_og_image',  true );
        $s_mode     = get_post_meta( $post_id, '_ras_schema_mode', true );
        $s_type     = get_post_meta( $post_id, '_ras_schema_type', true );
        $noindex    = get_post_meta( $post_id, '_ras_noindex', true );
        ...
        $dup_title = self::has_duplicate_meta( '_ras_title_override', $title, $post_id )
                  || self::has_post_with_same_title( $title, $post_id );
        $dup_meta  = $meta_d && self::has_duplicate_meta( '_ras_meta_desc', $meta_d, $post_id );
        ...
        if ( $post_type === 'product' && class_exists( 'RAS_Woo_Bridge' ) && RAS_Woo_Bridge::is_active() ) {
            foreach ( RAS_Woo_Bridge::seo_checks( $post_id ) as $id => $r ) {
                $results[ $id ] = $r;
            }
        }
